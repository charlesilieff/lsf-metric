...
object Spark:
  ...
  private def eventSequenceByUserId(userIds: Iterable[String]): DataFrame =
    val actorGuidString = userIds.mkString("'", "', '", "'")
    val query           = s"""
WITH actor_guids AS (
  SELECT unnest(ARRAY[$actorGuidString]) AS actor_guid
)
"""
    spark
      .read
      ...
      .option(
        "query",
        s"${query} select * from public.sessioninteractionswithautoincrementid where actorguid IN (SELECT actor_guid FROM actor_guids)")
      .load()

//  // 5 minutes in milliseconds
  val TIME_OUT = 300000

  def getSessionTimeByUserId(actorGuid: Iterable[String]): Iterable[UserSessionsTime] =
    ...
    val result = all.collect.groupBy(_._1).map {
      case (userId, values) =>
        val durationInSeconds = LocalTime.parse(values.head._3).toSecondOfDay

        UserSessionsTime(userId, values.head._2, Duration(durationInSeconds, SECONDS), values.head._4, values.head._5)
    }
    result
