...
import fr.rebaze.models.{Interaction, User, UserFirstnameAndLastname, Session => SessionModel}
import io.getquill.jdbczio.Quill
import io.getquill.{CamelCase, JsonValue, Query, Quoted}
...
final case class SessionRepositoryLive(quill: Quill.Postgres[CamelCase]) extends SessionRepository {
  ...
  private def querySession = quote(
  ...
  override def getRulesProgressByUserId(userId: String): Task[RulesProgressByUserId] = {
    ...
    val progressByUser        =
      progress
        .map(value =>
          value
            .groupBy(_.actorGuid).headOption.map {
              case (userId, interactions) =>
                (
                  userId,
                  interactions.map(interaction =>
                    (interaction.interaction.value.ruleId, interaction.interaction.value.progress.getOrElse(0.0))))
            })
        .map(value => value.getOrElse((userId, List.empty)))
    ...
    val rulesProgressByUserId = maxProgressByRuleId
      .map { case (userId, interactions) => RulesProgressByUserId(userId, interactions.toMap) }
